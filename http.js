...
module.exports = function (serverLog) {
  ...
  return function (request, response) {
    ...
    var match = /^\/replicate\/([a-f0-9]{64})$/.exec(pathname)
    if (match) {
      var projectDiscoveryKey = match[1]
      if (method === 'POST') {
        return replicate(projectDiscoveryKeys, request, response)
      }
      return respond405(request, response)
    }
...
function replicate (projectDiscoveryKey, request, response) {
  var log = request.log
  data.getProjectKeys(projectDiscoveryKey, function (error, keys) {
    ...
    request.pipe(protocol)
    request.once('end', function () {
      protocol.pipe(response)
      offerEnvelopes()
    })
  })
}
